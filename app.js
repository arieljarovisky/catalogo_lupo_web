...
function card(p) {
  ...
  return `<article class="product"><div class="thumb"><img loading="lazy" src="${p.image}" alt="${escapeHtml(translateText(p.name))}"><span class="badge">${escapeHtml(p.code)}</span></div><div class="info"><h4>${escapeHtml(translateText(p.name))}</h4><div class="price-row"><span>FOB</span><strong>${escapeHtml(formatFob(p.fobUsd))}</strong></div><div class="meta"><span class="pill">${escapeHtml(translateText(p.category))}</span><span class="pill">${escapeHtml(translateText(p.catalog))}</span></div><p class="desc">${escapeHtml(translateText(p.description || 'Sin descripci?n cargada.'))}</p><div class="swatches">${colorPreview || '<span class="swatch">Sin colores detectados</span>'}</div><div class="card-actions"><button class="btn primary" data-open="${p.id}">Ver detalle</button><a class="btn" href="${p.pdf}#page=${p.page}" target="_blank">PDF</a></div></div></article>`;
}
